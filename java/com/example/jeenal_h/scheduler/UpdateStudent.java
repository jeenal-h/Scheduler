            ...
            public void onClick(View v) {
                ...
                firebaseFirestore.collection("Users").document("Details")
                    ...
                    public void onSuccess(Void aVoid) {
                        Toast.makeText(UpdateStudent.this, "Student Details Updated", Toast.LENGTH_SHORT).show();
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        String error = e.getMessage();
                        Toast.makeText(UpdateStudent.this, "Error: " + error, Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                if (S_name.isEmpty() || S_class.isEmpty() || S_mobile.isEmpty() || S_email.isEmpty() || S_password.isEmpty() || S_type.isEmpty()) {
                    Toast.makeText(UpdateStudent.this, "Please enter all the details", Toast.LENGTH_SHORT).show();
                } else if (S_password.length() < 8) {
                    Toast.makeText(UpdateStudent.this, "Password should be greater than or equal to 8 characters", Toast.LENGTH_SHORT).show();
                } else if (S_mobile.length() < 10) {
                    Toast.makeText(UpdateStudent.this, "Mobile number should be of minimum 10 digits", Toast.LENGTH_SHORT).show();
                } else {
                    firebaseAuth.createUserWithEmailAndPassword(S_email, S_password)
                            .addOnCompleteListener(UpdateStudent.this, new OnCompleteListener<AuthResult>() {
                                @Override
                                public void onComplete(@NonNull Task<AuthResult> task) {
                                    if (task.isSuccessful()) {
                                        //Toast.makeText(UpdateStudent.this, "Student Registered Successfully", Toast.LENGTH_SHORT).show();
                                        onBackPressed();
                                    }
                                }
                            });
