            ...
            public void onClick(View v) {
                ...
                firebaseFirestore.collection("Users").document("Details")
                    ...
                    public void onSuccess(Void aVoid) {
                        Toast.makeText(UpdateTeacher.this,"Teacher Details Added",Toast.LENGTH_SHORT).show();
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        String error = e.getMessage();
                        Toast.makeText(UpdateTeacher.this,"Error: " +error,Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                if(T_name.isEmpty() || T_designation.isEmpty() || T_mobile.isEmpty() || T_email.isEmpty() || T_type.isEmpty()){
                    Toast.makeText(UpdateTeacher.this,"Please enter all the details",Toast.LENGTH_SHORT).show();
                }
                else if(T_password.length()<8){
                    Toast.makeText(UpdateTeacher.this,"Password should be greater than or equal to 8 characters",Toast.LENGTH_SHORT).show();
                }
                else if(T_mobile.length()<10){
                    Toast.makeText(UpdateTeacher.this,"Mobile number should be of minimum 10 digits",Toast.LENGTH_SHORT).show();
                }
                else {
                    firebaseAuth.createUserWithEmailAndPassword(T_email, T_password)
                            .addOnCompleteListener(UpdateTeacher.this, new OnCompleteListener<AuthResult>() {
                                @Override
                                public void onComplete(@NonNull Task<AuthResult> task) {
                                    if(task.isSuccessful()){
                                        //Toast.makeText(UpdateTeacher.this,"Teacher Registered Successfully", Toast.LENGTH_SHORT).show();
                                        onBackPressed();
                                    }
